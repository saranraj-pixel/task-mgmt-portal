  ...
  <div className="min-h-screen bg-gray-50 px-4 sm:px-6 lg:px-8 py-6 space-y-8">
    ...
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      <button
        ...
        className="flex items-center cursor-pointer justify-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-xl hover:bg-blue-700 transition shadow-sm"
      >
        <FiPlus />
        Create Task
      </button>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Panel title="Tasks by Status">
        {data.tasksByStatus?.map((item) => (
          <ProgressRow key={item._id} label={item._id} value={item.count} />
        ))}
      </Panel>
    ...
    <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
      ...
      <div className="md:hidden divide-y">
        {data.usersWithTasks?.map((user) => (
          <div key={user._id} className="p-4 space-y-1">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Panel title="Recent Users">
        {data.recentUsers?.map((u) => (
          <div key={u._id} className="py-2 border-b last:border-none">
      ...
      <Panel title="Recent Tasks">
        {data.recentTasks?.map((t) => (
          <div key={t._id} className="py-2 border-b last:border-none">
...
const StatCard = ({ title, value, icon, color }) => (
  <div className={`p-5 rounded-2xl shadow-sm bg-linear-to-r ${color} text-white hover:scale-[1.02] transition`}>
